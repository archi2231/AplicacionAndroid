...
public class EstadoCuentaDAO extends GenericDAO {
	...
	public List<IFEstadoDeCuentaGenerico> getRegistros() {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				String tipoCuenta = cursor.getString(cursor
						.getColumnIndex("tipoDeCuenta"));
	...
	private IFEstadoDeCuentaGenerico llenaEstadoCuenta(Cursor cursor, IFEstadoDeCuentaGenerico estado) {
		...
		if (IFReestructuraGenerico.class.isAssignableFrom(estado.getClass())) {
			...
			((IFReestructuraGenerico) estado).setUltimoPagoInfonavit(cursor
					.getString(i++));
			((IFReestructuraGenerico) estado).setUltimoPagoInfonavitVSM(cursor
					.getString(i++));
			((IFReestructuraGenerico) estado).setSaldoACargoAcreditado(cursor
					.getString(i++));
			((IFReestructuraGenerico) estado)
					.setSaldoACargoAcreditadoVSM(cursor.getString(i++));
			((IFReestructuraGenerico) estado).setSaldoACargoInfonavit(cursor
					.getString(i++));
			((IFReestructuraGenerico) estado).setSaldoACargoInfonavitVSM(cursor
					.getString(i++));
			((IFReestructuraGenerico) estado).setSaldoActualReestructura(cursor
					.getString(i++));
			((IFReestructuraGenerico) estado)
					.setSaldoActualReestructuraVSM(cursor.getString(i++));
		...
		if (IFR40Generico.class.isAssignableFrom(estado.getClass())) {
			i = 41;
			((IFR40Generico) estado).setDescuentoTasaInteres(cursor
					.getString(i++));
			((IFR40Generico) estado).setTasaInteresAnualCobrada(cursor
					.getString(i++));
			((IFR40Generico) estado).setInteresesGeneradosEnPeriodo(cursor
					.getString(i++));
			((IFR40Generico) estado).setInteresesCubiertosInfonavit(cursor
					.getString(i++));
			((IFR40Generico) estado).setInteresesCubiertosInfonavitVSM(cursor
					.getString(i++));
			((IFR40Generico) estado).setInteresesCubiertosAcreditado(cursor
					.getString(i++));
			((IFR40Generico) estado).setInteresesCubiertosAcreditadoVSM(cursor
					.getString(i++));
			((IFR40Generico) estado).setTasaInteresInicioAplicada1(cursor
					.getString(i++));
			((IFR40Generico) estado).setTasaInteresFinAplicada1(cursor
					.getString(i++));
			((IFR40Generico) estado).setTasaInteresInicioAplicada2(cursor
					.getString(i++));
			((IFR40Generico) estado).setTasaInteresFinAplicada2(cursor
					.getString(i++));
	...
	public void insertaRegistro(IFEstadoDeCuentaGenerico estadoCuenta) {

		try {
			String query = construyeQueryInsert(estadoCuenta);
	...
	private String construyeQueryInsert(IFEstadoDeCuentaGenerico estadoCuenta) {
		...
		StringBuilder campos = new StringBuilder(
				"idUsuario, numBoletos, mensajeCartera, mensajeGeneral, periodo, tipoDeCuenta, nombreCliente, noCredito, pagoContratado, tasaInteresAnual, fechaOtorgamiento, noDeMensualidadesVencidas, nivelCumplimientoPago, montoVencido, saldoAnterior, saldoAnteriorVSM, ajustePeriodosAnteriores, ajustePeriodosAnterioresVSM, saldoAjustado, saldoAjustadoVSM, interesesPeriodo, interesesPeriodoVSM, comisionesOtrosCargos, comisionesOtrosCargosVSM, ultimoPago, ultimoPagoVSM, traspasosOtrosAbonos, traspasosOtrosAbonosVSM, saldoActualResumen, saldoActualVSMResumen");
		UsuarioDTO user = UsuarioDTO.getUniqueUser();
//		String.format(locale, format, args);
		String strDatos = String
				.format(new Locale("es","MX"),"%d,\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"",
						user.getId(), estadoCuenta.getBoletos(),
						estadoCuenta.getMensajeCartera(),
						estadoCuenta.getMensajeGeneral(),
						estadoCuenta.getPeriodo(),
						estadoCuenta.getTipoCuenta(), estadoCuenta.getNombre(),
		...
		if (estadoCuenta instanceof IFREANormal) {
			campos.append(", lineaDeCaptura");
			datos.append(String.format(",\"%s\"",
					((IFREANormal) estadoCuenta).getLineaCaptura()));
		}

		if (estadoCuenta instanceof IFROANormal) {
			campos.append(", pagoDeTuPatron, pagoDeTuPatronVSM");
			datos.append(String.format(",\"%s\",\"%s\"",
					((IFROANormal) estadoCuenta).getpagoDeTuPatron(),
					((IFROANormal) estadoCuenta).getPagoDeTuPatronVSM()));
		}

		if (IFReestructuraGenerico.class.isAssignableFrom(estadoCuenta
				.getClass())) {
			...
			datos.append(String.format(
					",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"",
					((IFReestructuraGenerico) estadoCuenta)
							.getUltimoPagoInfonavit(),
					((IFReestructuraGenerico) estadoCuenta)
							.getUltimoPagoInfonavitVSM(),
					((IFReestructuraGenerico) estadoCuenta)
							.getSaldoACargoAcreditado(),
					((IFReestructuraGenerico) estadoCuenta)
							.getSaldoACargoAcreditadoVSM(),
					((IFReestructuraGenerico) estadoCuenta)
							.getSaldoACargoInfonavit(),
					((IFReestructuraGenerico) estadoCuenta)
							.getSaldoACargoInfonavitVSM(),
					((IFReestructuraGenerico) estadoCuenta)
							.getSaldoActualReestructura(),
					((IFReestructuraGenerico) estadoCuenta)
							.getSaldoActualReestructuraVSM()));
		...
		if (IFR40Generico.class.isAssignableFrom(estadoCuenta.getClass())) {
			...
			datos.append(String
					.format(",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"",
							((IFR40Generico) estadoCuenta)
									.getDescuentoTasaInteres(),
							((IFR40Generico) estadoCuenta)
									.getTasaInteresAnualCobrada(),
							((IFR40Generico) estadoCuenta)
									.getInteresesGeneradosEnPeriodo(),
							((IFR40Generico) estadoCuenta)
									.getInteresesGeneradosEnPeriodoVSM(),
							((IFR40Generico) estadoCuenta)
									.getInteresesCubiertosInfonavit(),
							((IFR40Generico) estadoCuenta)
									.getInteresesCubiertosInfonavitVSM(),
							((IFR40Generico) estadoCuenta)
									.getInteresesCubiertosAcreditado(),
							((IFR40Generico) estadoCuenta)
									.getInteresesCubiertosAcreditadoVSM(),
							((IFR40Generico) estadoCuenta)
									.getTasaInteresInicioAplicada1(),
							((IFR40Generico) estadoCuenta)
									.getTasaInteresFinAplicada1(),
							((IFR40Generico) estadoCuenta)
									.getTasaInteresInicioAplicada2(),
							((IFR40Generico) estadoCuenta)
									.getTasaInteresFinAplicada2()));
		}

		String query = "INSERT INTO " + getNombreTabla() + " (" + campos
				+ ") VALUES (" + datos + ")";
	...
	public void borraRegistro(long id) {
		...
		Log.i("Eliminaci—n Estado Cuenta", "Code: " + code);
		// if(result > 0)
		// return true;
		// return false;
	}
